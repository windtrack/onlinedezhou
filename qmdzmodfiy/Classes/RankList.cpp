...
{
	ERankWeeksRecordMenuTag,	// 周战绩榜
	EWeeksProfitMenuTag,		// 周盈利榜
	EDaySingleMenuTag,			// 日单局榜
	ERichMenuTag				// 财富榜
};
...
//---------------------------------------------------------------------------------------------------------------
void RankList::onSetVisible(bool bVisible)
{
	if (bVisible)
	{
...
bool RankList::init()
{
	...
	// 好友排名的按钮回调
	m_pGoodFriendMenu = MenuItemSpriteWithLabel::create("ranklist/goodlist.png", 3, CC_CALLBACK_1(RankList::GoodFriendMenuCallback, this, 0));
	...
	// 世界排名的按钮回调
	m_pTheWorldMenu = MenuItemSpriteWithLabel::create("ranklist/worldlist.png", 3, CC_CALLBACK_1(RankList::GoodFriendMenuCallback, this, 1));
	...
	for(int i = 0;i < 4; i++)
	{
		m_pFriendsWorldMenu[i] = MenuItemSpriteWithLabel::create("messagelist/messagebutton.png", 3, CC_CALLBACK_1(RankList::AllMenuCallback, this, i));
	...
	auto nTheWorldMenu1 = CreateLabelMSYHBD(26,"周战绩榜");
	...
	auto nTheWorldMenu2 = CreateLabelMSYHBD(26,"周盈利榜");
	...
	auto nTheWorldMenu3 = CreateLabelMSYHBD(26,"日单局榜");
	...
	auto nTheWorldMenu4 = CreateLabelMSYHBD(26,"财富榜");
	m_pFriendsWorldMenu[3]->SetLabel(nTheWorldMenu4);
	m_pFriendsWorldMenu[3]->SetEnabledColor(cocos2d::Color4B(0x37,0xa4,0x5c,0xff));
	m_pFriendsWorldMenu[3]->SetDisabledColor(cocos2d::Color4B(0xff,0xff,0xff,0xff));
...
//---------------------------------------------------------------------------------------------------------------
// 好友和世界排名的回调函数
void RankList::GoodFriendMenuCallback(Ref* pSender, int num)
{
	if(num == 0)
	{
	...
	for(int i = 0; i < 4; i++)
	{
		if(i == m_uRecordMenuCurrentStatusId)
		{
			...
			if(m_uRecordMenuCurrentStatusId == ERankWeeksRecordMenuTag)	// 当记录好友世界排名和按钮事件状态都为0时
			{
...
// 好友列表下按钮的所有回调函数
void RankList::AllMenuCallback(Ref* pSender, int num)
{
	for(int i = 0;i < 4;i++)
	{
		if(i == num)
...
// 初始化底池周战绩排行榜
void RankList::FriendWorldWeeksRecordRichList(unsigned uRankType, unsigned uWinType)
{
	...
	// 对上一页、下一页的按钮的位置设置
	uGoodFriendFirstRankNum = g_globalRankUserInfo.GetRankList(uRankType, uWinType).size();	// 得到1~3的容器的数量
	...
	g_globalRankUserInfo.GetPageInfo(RecoderuRankType, RecoderuWinType, nServerCurrentPage, nServerMaxPage);	// 得到当前页数
    ...
    /*if (nServerCurrentPage == 1 && nServerMaxPage == 1)
    {
    uAllItemH += size.height;
    }
    if (nServerCurrentPage == 1 && nServerMaxPage == 0)
    {
    uAllItemH += size.height;
    }*/
	...
	if (nPageDown)
	{
		...
		m_nPageDown->SetHitCB(std::bind(&RankList::PageDownMenu, this, nServerCurrentPage, nServerMaxPage));	// 下一页的按钮的回调函数
		fY += uItemH / 2 + fY;
	}
	else
	{
	...
	// vector的倒序遍历
	auto&& v = g_globalRankUserInfo.GetRankList(uRankType, uWinType);
	for(auto it = v.rbegin(); it != v.rend(); it++ )
	{
		...
		if (nServerCurrentPage > 1)
		{
			...
			m_nPageUp->SetHitCB(std::bind(&RankList::PageUpMenu, this, nServerCurrentPage, nServerMaxPage));
		}
		else
		{
			m_nPageUp->setVisible(false);
...
// 初始化奖金周盈利、日单局、财富榜排行榜
void RankList::FriendWorldRemainRichList(unsigned uRankType, unsigned uWinType)
{
	bool nPageUp = false, nPageDown = false, nPageSmall = false;
	ClearRemainRichList();
	ClearWeeksRecordRichList();
	...
	RecoderuWinType = uWinType;		// 记录按钮的值
	// 对上一页、下一页的按钮的位置设置
	uFriendWorldRemainRankNum = g_globalRankUserInfo.GetRankList(uRankType, uWinType).size();			// 得到1~3的容器的数量
	...
	if (nPageDown)		// 是否下一页
	{
		...
		m_nPageDown->setVisible(true);
		m_nPageDown->setPosition(fX, fY);
		m_nPageDown->SetHitCB(std::bind(&RankList::PageDownMenu, this, nServerCurrentPage, nServerMaxPage));
		fY += uItemH / 2 + fY;
	}
	else
	{
	...
	// vector的倒序遍历
	auto&& v = g_globalRankUserInfo.GetRankList(uRankType, uWinType);			// 奖金周盈利、日单局、财富榜的Vector遍历
	for(auto it = v.rbegin(); it != v.rend(); it++ )
	{
	...
	if (nServerCurrentPage > 1)
	{
		...
		m_nPageUp->setVisible(true);
		m_nPageUp->setPosition(fX, fY);
		m_nPageUp->SetHitCB(std::bind(&RankList::PageUpMenu, this, nServerCurrentPage,nServerMaxPage));	// 上一页的按钮的回调函数
	}
	else
	{
		m_nPageUp->setVisible(false);
...
//---------------------------------------------------------------------------------------------------------------
// 上一页
void RankList::PageUpMenu(unsigned nServerCurrentPage, unsigned nServerMaxPage)
{
	Json::Value param;
	...
	param["pageCount"] = 10;
	SendClientRequest(ClientRequest::kGetRankList, param);
}

//---------------------------------------------------------------------------------------------------------------
// 下一页
void RankList::PageDownMenu(unsigned nServerCurrentPage, unsigned nServerMaxPage)
{
	Json::Value param;
	param["winType"] = RecoderuWinType;
	param["rankType"] = RecoderuRankType;
...
//---------------------------------------------------------------------------------------------------------------
// 获取玩家奖金的列表
RankList::UserWinPotListItem* RankList::GetUserWinPotListItem(unsigned m_uUserWinPotID)
{
...
//---------------------------------------------------------------------------------------------------------------
RankList::UserWinCashListItem* RankList::GetUserWinCashListItem(unsigned m_uUserWinCashID)					// 获取玩家底池的列表
{
...
//---------------------------------------------------------------------------------------------------------------
// 更新奖金的列表 
void RankList::UpdateUserWinPotListItem(RankList::UserWinPotListItem *item, const RankUserInfo& rankuserinfo)
{
	// 得到当前的玩家的称呼
...
//---------------------------------------------------------------------------------------------------------------
// 更新底池的列表
void RankList::UpdateUserWinCashListItem(RankList::UserWinCashListItem *item, const RankUserInfo& rankuserinfo)
{
	// 得到当前的玩家的称呼
	SetLabelString(item->m_pLabelName, rankuserinfo.m_strNickName);	
...
//---------------------------------------------------------------------------------------------------------------
// 通知更新函数
void RankList::UpdateRankList(unsigned uRankType, unsigned uWinType)
{
	// 当前是好友和世界列表周战绩排名
...
// 清理所有的底池的vector
void RankList::ClearWeeksRecordRichList()
{
...
// 清理所有的奖金的vector
void RankList::ClearRemainRichList()
{
...
void RankList::setVisible(bool bVisible)
{
