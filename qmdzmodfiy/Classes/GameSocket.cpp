...
{  
	// 初始化 
	m_bufInput = new char[INBUFSIZE];
	memset(m_bufInput, 0, INBUFSIZE); 
...
bool CGameSocket::Create(const char* pszServerIP, int nServerPort, int nBlockSec, bool bKeepAlive ) 
{ 
	// 检查参数 
...
bool CGameSocket::SendMsg(const void* pBuf, int nSize) 
{ 
	if(pBuf == 0 || nSize <= 0) { 
		return false; 
	} 
...
//		(unsigned char)m_bufInput[(m_nInbufStart + 1) % INBUFSIZE] * 256; // 注意字节序，高位+低位 
	...
	{
		temp[i] = m_bufInput[(m_nInbufStart+i)%INBUFSIZE];
	}
	int packsize = ntohl(*(unsigned long*)temp);
